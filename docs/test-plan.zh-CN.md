...
| 手动关闭优先于自动展开 | 当前页面位于一个会被自动展开的分支里 | 先关闭这个分支，再在同一侧栏里展开兄弟目录 | 已被用户手动关闭的分支保持关闭，不会在兄弟目录展开引发的重渲染里自己重新打开 |
...
- 手动关闭覆盖自动展开约束：`node tests/validate-manual-collapse-beats-auto-open.mjs`
