...
- 仓库外的别的目录会启动不同端口
- 如果已经装过 Codex app patch，后续执行普通 `bash install.sh` / `--vscode` / `--finder` 也会顺手同步 `codex-app` runtime，不需要每次重新打 patch
...
- 从 Codex app 里打开预览还是旧行为：先执行一次 `bash install.sh`，它会自动刷新已安装的 `~/Library/Application Support/Use Browser Priview/codex-app` runtime
