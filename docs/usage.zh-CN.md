...
- 如果没有找到项目标记，VS Code / Codex 预览最多只会向上退到 `~/`
- 如果一路往上都没有这些标记，就把当前选中的目录本身当作根
- 同一个项目根在 Finder 和 VS Code / Codex 之间共用一个预览端口
- 运行时代码升级后，只要旧端口能被回收，同一个项目根仍然继续使用原端口
- 预览后端现在由独立 supervisor 常驻，浏览器标签页、Finder 和 VS Code / Codex 不再需要一起活着才能维持这个端口
- 仓库外的别的目录会启动不同端口
- 如果 Codex app patch 是从当前本地仓库装上的，已安装的 Codex wrapper 会直接跟随这个仓库，后续 runtime 改动会自动生效
- 如果 Codex app patch 是从快照装上的，后续执行普通 `bash install.sh` / `--vscode` / `--finder` 也会顺手同步 `codex-app` runtime，不需要每次重新打 patch
...
其中 `sm` 现在是超紧凑的小号字，适合图片表格里的赞藏数据、辅助说明这类密集元信息，优先服务于避免统计文字换行这种场景。
...
如果 Markdown 表格本身就是 `search-results.images.md` 这种图片浏览网格，预览器会自动把图片下方的统计行切到更小、更紧凑且不换行的元信息布局，尽量把赞藏数据保持在一行里。

## 常见问题

- VS Code 里首次安装后还是没看到菜单：重开当前 VS Code / Codex 窗口一次
- 运行时代码已经更新，但下一次预览看起来还是旧的：再触发一次 `Use Browser Priview`，让当前适配器切到最新 runtime
- 半夜或宿主重载后预览挂掉：先看 `~/Library/Application Support/Use Browser Priview/preview-supervisor.log`，确认是不是 supervisor 或预览子进程被系统终止
- 从 Codex app 里打开预览还是旧行为：本地仓库安装通常会在下一次打开时自动跟上；如果是快照安装，再执行一次 `bash install.sh`，它会自动刷新已安装的 `~/Library/Application Support/Use Browser Priview/codex-app` runtime
