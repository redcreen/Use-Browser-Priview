...
如果这条 patch 是从当前本地仓库装上的，已安装的 Codex wrapper 会保持一条回到本地仓库的 live link，后续仓库里的 runtime 改动会自动被它吃到。只有快照安装场景，后续执行普通 `bash install.sh`、`bash install.sh --vscode` 或 `bash install.sh --finder` 时，才需要顺手同步 `~/Library/Application Support/Use Browser Priview/codex-app` 里的 runtime；这仍然不需要每次重新打 patch。
...
- VS Code / Codex 文件预览如果一路往上都没有这些标记，最多只会向上退到 `~/`
- Finder 文件夹项如果没有项目标记，仍然以当前选中的文件夹本身作为根
...
- Codex app 里还是旧行为：如果 patch 是从当前本地仓库装上的，重新打开一次预览就应该跟上仓库最新代码；如果是快照安装，再执行一次普通 `bash install.sh` 刷新已安装的 `codex-app` runtime；只有 Codex 自己刚更新过，才需要重新安装一次 patch
