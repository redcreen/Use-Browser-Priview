...
一旦这条 patch 已经装过，后续执行普通 `bash install.sh`、`bash install.sh --vscode` 或 `bash install.sh --finder` 时，也会顺手同步 `~/Library/Application Support/Use Browser Priview/codex-app` 里的 runtime，不需要每次重新打 patch。
...
- Codex app 里还是旧行为：先执行一次普通 `bash install.sh` 刷新已安装的 `codex-app` runtime；如果 Codex 刚更新过，再重新安装一次 patch
